...
struct view_info s_info = { .win = NULL, .conform = NULL, .navi = NULL, .buttons = { NULL }, };
 ...
 *
 */
void _create_button(app_button button, Evas_Object *display, char *name,
		void *callback) {
	s_info.buttons[button] = _new_button((void *) button, display, name,
			callback);
}
 ...
 */
Evas_Object *_new_button(void *data, Evas_Object *display, char *name,
		void *callback) {
...
static void _btn_clear_cb(void *data, Evas_Object *object, void *event_info) {
	...
	_image_util_clear_cb(data, object, event_info);
}
...
Evas_Object *_create_new_cd_display(char *name, void *callback) {
	...
	if (callback != NULL)
		elm_naviframe_item_pop_cb_set(item,
				(Elm_Naviframe_Item_Pop_Cb) callback, NULL);
	else
	...
	evas_object_size_hint_align_set(bt, EVAS_HINT_FILL, EVAS_HINT_FILL);
	evas_object_size_hint_weight_set(bt, EVAS_HINT_EXPAND, 0.0);
	evas_object_smart_callback_add(bt, "clicked", _btn_clear_cb, callback);
	evas_object_show(bt);
	...
	/* Create a width input entry */
	Evas_Object *width = elm_entry_add(box);
	s_info.width = width;
	evas_object_size_hint_align_set(width, EVAS_HINT_FILL,
			EVAS_HINT_FILL);
	evas_object_size_hint_weight_set(width, EVAS_HINT_EXPAND,
			0.0);
	evas_object_show(width);

	elm_entry_scrollable_set(width, EINA_TRUE);
	elm_entry_single_line_set(width, EINA_TRUE);
	elm_object_part_text_set(width, "elm.guide", "insert width");

	elm_scroller_policy_set(width, ELM_SCROLLER_POLICY_OFF,
			ELM_SCROLLER_POLICY_ON);

	/* Create a height input entry */
	Evas_Object *height = elm_entry_add(box);
	s_info.height = height;
	evas_object_size_hint_align_set(height, EVAS_HINT_FILL,
			EVAS_HINT_FILL);
	evas_object_size_hint_weight_set(height, EVAS_HINT_EXPAND,
			0.0);
	evas_object_show(height);

	elm_entry_scrollable_set(height, EINA_TRUE);
	elm_entry_single_line_set(height, EINA_TRUE);
	elm_object_part_text_set(height, "elm.guide", "insert height:");

	elm_scroller_policy_set(height, ELM_SCROLLER_POLICY_OFF,
			ELM_SCROLLER_POLICY_ON);

	elm_box_pack_end(box, width);
	elm_box_pack_end(box, height);
 ...
 */
Evas_Object *view_create_layout(Evas_Object *parent, const char *file_path,
